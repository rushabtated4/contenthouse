...
- **Image Processing:** sharp (resize 1080x1350, strip metadata, SVG text composite), ffmpeg-static + fluent-ffmpeg (video trimming)
...
| `hook_sessions` | Hook Creator 6-step wizard sessions (0-5). Stores input video, snapshot frame, wizard step, and final config. Has `trimmed_video_url`, `trimmed_thumbnail_url`, `trimmed_duration` for auto-saved clips. `source_session_id` FK for clip provenance. `source_type` includes `clip`. TikTok stats: `tiktok_play_count`, `tiktok_digg_count`, `tiktok_comment_count`, `tiktok_share_count`, `tiktok_collect_count`. |
...
| `demo_videos` | Demo video clips for hook compositions. Has `video_url`, `thumbnail_url`, `title`, `duration`, `file_size`. |
| `hook_compositions` | Edited hook videos with text overlays + demo concat. FK `source_video_id` → `hook_generated_videos`, FK `demo_video_id` → `demo_videos`, FK `channel_id` → `project_accounts`. Has `text_overlays` JSONB, `rendered_video_url`, `status` (draft/rendering/completed/failed), `review_status`, scheduling fields. |
...
| `/api/hooks/sessions/[id]/select-images` | PATCH | Mark selected images (or import from library) and advance wizard step |
| `/api/hooks/sessions/[id]/generate-videos` | POST | Generate hook videos via Replicate Kling v2.6 |
| `/api/hooks/sessions/[id]/trim` | POST | Server-side FFmpeg trim + thumbnail extraction |
...
| `/api/hooks/images/library` | GET | List all completed hook images across sessions |
...
| `/api/demo-videos` | GET/POST | List/upload demo videos |
| `/api/demo-videos/[id]` | PATCH/DELETE | Rename/delete demo video |
...
- Route-level `export const maxDuration = 300` on `/api/queue`, `/api/generate/[imageId]/retry`, `/api/editor/generate-background/batch`, `/api/backgrounds/upload-batch`, and `/api/hooks/sessions/[id]/trim`

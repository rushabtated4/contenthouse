...
| `source_type` | text | NO | — | `'tiktok'`, `'upload'`, or `'clip'` |
...
| `trimmed_video_url` | text | YES | — | Storage URL of trimmed clip in `hook-videos/trimmed/` |
| `trimmed_thumbnail_url` | text | YES | — | Storage URL of thumbnail in `hook-images/thumbnails/` |
| `trimmed_duration` | real | YES | — | Duration of trimmed clip in seconds |
| `source_session_id` | uuid | YES | — | FK → `hook_sessions.id` (SET NULL on delete). Provenance for `clip` source type |
| `tiktok_play_count` | bigint | YES | — | TikTok video play/view count |
| `tiktok_digg_count` | bigint | YES | — | TikTok video like count |
| `tiktok_comment_count` | bigint | YES | — | TikTok video comment count |
| `tiktok_share_count` | bigint | YES | — | TikTok video share count |
| `tiktok_collect_count` | bigint | YES | — | TikTok video save/collect count |

**Indexes:**
- PK on `id`
- `idx_hook_sessions_status` on `status`
- `idx_hook_sessions_created_at` on `created_at DESC`
- `idx_hook_sessions_trimmed` on `created_at DESC` WHERE `trimmed_video_url IS NOT NULL`

**Constraints:**
- `hook_sessions_source_type_check` CHECK `source_type IN ('upload', 'tiktok', 'clip')`
...
| `model` | text | NO | `'google/nano-banana-pro'` | Replicate model used for generation |
...
Demo/product videos that can be appended to hook compositions.

| Column | Type | Nullable | Default | Description |
|---|---|---|---|---|
| `id` | uuid | NO | `gen_random_uuid()` | Primary key |
| `video_url` | text | NO | — | Storage URL of the demo video |
| `thumbnail_url` | text | YES | — | Storage URL of extracted thumbnail |
| `title` | text | YES | — | User-defined title |
...
### `hook_compositions` (new)

Compositions combining a hook video with text overlays and an optional demo video. Supports scheduling and posting.

| Column | Type | Nullable | Default | Description |
|---|---|---|---|---|
| `id` | uuid | NO | `gen_random_uuid()` | Primary key |
| `source_video_id` | uuid | NO | — | FK → `hook_generated_videos.id` (CASCADE delete) |
| `demo_video_id` | uuid | YES | — | FK → `demo_videos.id` (SET NULL on delete) |
| `rendered_video_url` | text | YES | — | Storage URL of rendered composition |
| `thumbnail_url` | text | YES | — | Storage URL of composition thumbnail |
| `text_overlays` | jsonb | YES | `'[]'` | Array of VideoTextOverlay objects |
| `duration` | real | YES | — | Total composition duration in seconds |
| `status` | text | NO | `'draft'` | `draft` / `rendering` / `completed` / `failed` |
| `error_message` | text | YES | — | Error details if status = failed |
| `review_status` | text | NO | `'unverified'` | `unverified` / `ready_to_post` |
| `channel_id` | uuid | YES | — | FK → `project_accounts.id` (SET NULL on delete) |
| `scheduled_at` | timestamptz | YES | — | When to post |
| `posted_at` | timestamptz | YES | — | When marked as posted |
| `notes` | text | YES | — | User notes for scheduling |
| `created_at` | timestamptz | NO | `now()` | Record creation |
| `updated_at` | timestamptz | NO | `now()` | Last update |
...
**Indexes:**
- PK on `id`
- `idx_hook_compositions_source` on `source_video_id`
- `idx_hook_compositions_scheduled` on `scheduled_at` WHERE `scheduled_at IS NOT NULL`

**Constraints:**
- `hook_compositions_status_check` CHECK `status IN ('draft', 'rendering', 'completed', 'failed')`
- `hook_compositions_review_status_check` CHECK `review_status IN ('unverified', 'ready_to_post')`
...
hook_sessions
  ...
  └──< hook_generated_videos.session_id (CASCADE delete)
          ...
          │
          └──< hook_compositions.source_video_id (CASCADE delete)
                  │
                  ├── hook_compositions.demo_video_id >── demo_videos (SET NULL)
                  └── hook_compositions.channel_id >── project_accounts (SET NULL)
```
